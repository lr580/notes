...
> ## 题目描述
>
> 你站在二维平面整数点 $(0,0)$。你希望恰好进行 $k$ 次跳跃到达整数点 $(x,y)$，并且每次跳跃的欧式距离都完全相同且不为 0。
>
> 请给出每一次跳跃结束后到达的整数点坐标序列（共 $k$ 个点，最后一个应为 $(x,y)$）。若无解，请输出 `NO`。
>
> > 【说明】欧式距离指两点间的直线距离：从 $(x_1,y_1)$ 跳到 $(x_2,y_2)$ 的长度为：
> >
> > $$
...
> ## 输入描述
>
> 每个测试文件包含多组测试数据。第一行输入一个整数 $T$ $(1 \le T \le 10^5)$ 表示测试组数。
>
> 随后每组数据一行输入三个整数：
>
> $$
> x,\ y,\ k\quad (-10^9 \le x,y \le 10^9,\ 1 \le k \le 2\times10^5)
> $$
>
> 保证所有测试中 $k$ 的总和不超过 $5\times10^5$。
>
> ## 输出描述
>
> 对于每组测试数据：
>
> - 若无解，输出一行 `NO`。
> - 若有解，先输出一行 `YES`，随后输出 $k$ 行，每行两个整数，用一个空格隔开，依次为第 $1\sim k$ 次跳跃结束后到达的点坐标 $(x_i,y_i)$。要求：
>   - $x_k=x,\quad y_k=y$
>   - 坐标范围约束：若存在可行解，所输出的每次跳跃结束坐标 $(x_i,y_i)$ 必须满足 $-2\times10^9 \le x_i,y_i \le 2\times10^9$。
>   - 各步的落点可重复，无需保证互不相同。
>
> 如果存在多个解法，你可以输出任意一个，系统会自动判定是否正确。注意，自测运行功能可能因此返回错误结果，请自行检查答案正确性。
>
> ## 示例 1
>
> 输入：
...
若是原点，是奇数无解。偶数随便找一格来回走。

否则，k奇数，在终点和原点之间来回走。

k=2：

1. 若 x, y 奇偶相同。以 (0,0), (x,y) 为对角线，做正方形的另一条对角线，其两点为解。如 (x-y)/2, (x+y)/2
2. 否则，必然无解。

k>2的偶数：先在原点和刚刚构造的点之间反复横跳，直到剩下最后两步。

证明：

设每一次跳跃的向量是(a1,b1), ... (ak, bk)
那么 x=a1+...+ak, y = b1+...+bk
若奇偶不同，x+y≡1 (mod2)
由于a1^2+b1^2=...=ak^2+bk^2
且任意n满足 n^2≡n (mod2)，所以 a1+b1≡a2+b2≡....≡ak+bk (mod2)
所以x+y≡k(a1+b1) (mod2)，因为k是偶数，不管a1+b1是奇数还是偶数，k(a1+b1)一定是偶数，不可能等于奇数，即x+y≡1 (mod2)必然无解。

其中构造方法是

对角线方向向量是(x,y)，跟它垂直即 x1x2+y1y2=0，可以构造出：另一个方向向量是(-y,x)或(y,-x) 
放缩这个向量长度为对角线半长(-y/2,x/2)，(y/2,-x/2) 然后以正方形中点(x/2,y/2)为起点加上这个向量， 得解为( (x-y)/2, (x+y)/2) 或 (  (x+y)/2, (y-x)/2 )
...
int main() {
    ...
    while (T--) {
        ...
        if (x == 0 && y == 0) {
            ...
            int64 g = gcd(llabs(x), llabs(y));

            int64 a = y / g;
            int64 b = -x / g;

            // x,y 都为偶数时，乘 2 保证后续除以 2 后仍是整数
            if ((x & 1) == 0) {
                a *= 2;
                b *= 2;
            }
...
我的赛时解法，但是爆ll了，没考虑到会超ll：

设中间点是 (a,b)，构造 a^2+b^2=(a-x)^2+(b-y)^2，解得：2ax+2by=x^2+y^2，求解二元一次不定方程，用exgcd。
 ...
 * 这里要求传入的 a,b 非负。
 */
int64 exgcd(int64 a, int64 b, int64 &p, int64 &q) {
    if (b == 0) {
        p = 1;
        q = 0;
        return a;
 ...
 * 返回最接近 n / d 的整数。
 *
 * 使用 __int128 防止扩展欧几里得的初始解乘上常数时溢出。
 */
i128 roundDivide(i128 n, i128 d) {
    if (d < 0) {
        n = -n;
        d = -d;
    }
...
int main() {
    ...
    while (T--) {
        ...
        if (x == 0 && y == 0) {
            ...
            i128 a0 = static_cast<i128>(px) * multiple;
            i128 b0 = static_cast<i128>(py) * multiple;
             ...
             *      b = b0 - (x/g)*t
             *
             * 初始解可能非常大，因此需要选择合适的 t，
             * 让 (a,b) 尽量靠近中点 (x/2,y/2)。
             *
             * 这也可以保证最终输出坐标不超过题目限制。
             */
            i128 dx = X / g;
            ...
            if (dy != 0) {
                 ...
                 *      t ≈ (2*b0-y)/(2*dx)
                 */
                t = roundDivide(2 * b0 - Y, 2 * dx);
            }

            i128 finalA = a0 + dy * t;
            i128 finalB = b0 - dx * t;

            a = static_cast<int64>(finalA);
            b = static_cast<int64>(finalB);
        }
...
> ## 题目描述
>
> 学校组织了一次集体合影活动，有 $N$ 个同学会站在 $N$ 级台阶上拍照，学号从 $1$ 到 $N$，每一级台阶的高度为 $D$。
>
> 为了美观，每一级台阶上都必须有一名同学。令 $p_i$ 表示站在从下往上第 $i$ 级台阶上同学的学号。
>
> 每个同学都有一个身高值，学号为 $i$ 的同学的身高用正整数 $A_i$ 表示。为了防止遮挡，对于所有 $1 \le i < N$，必须满足：
>
> $$
> A_{p_{i+1}} \ge A_{p_i} - D
> $$
>
> 现在给定 $N$ 个同学的身高序列 $A$，请计算有多少种不同的队列 $p$，使得同学按此方案站在台阶上面后满足上述拍照条件。
>
> 由于答案可能很大，请输出答案对 $998244353$ 取模的结果。
>
> ## 输入描述
>
> 第一行包含两个整数 $N$ 和 $D$，分别表示同学的数量和台阶高度。
>
> 第二行包含 $N$ 个整数 $A_1,A_2,\cdots,A_N$，表示每个同学的身高。
>
...
> ```
>
> 1 2 4 4, 1 2 4 4, 2 1 4 4, 2 1 4 4
...
signed main() {
    ...
    int ans = 1e18;
    if(true || n<=500) { // brute force, 73% at most, if true
        vector<ll> ans2mi(n+1, 1e18), ans2mx(n+1, 0), ans2(n+1, 1e18);
        for(int i=1;i<n;i++) {
            for(int j=i+1;j<=n;j++) {
                ll s1 = s[j] - s[i];
                ll s2 = s[n] - s[j];
        ...
        for(int k1=1;k1<n;k1++) {
            ...
            for(int k2=k1+1;k2<n;k2++) {
                ...
                ll mi2 = min(s1, s2);
                if(true) { // 73 -> 82%, 启发式剪枝
                    ll mi = min(mi2, ans2mi[k2]);
                    ll mx = max(mx2, ans2mx[k2]);
                    if(mx - mi < ans) {
                        ans = mx - mi;
                    }
                    continue;
    ...
    // DP? , at least O(n^2), how optim?
    vector<int> dp2(n+1, 0);
    // binary search, and how calc mid 2 blocks?
    return 0;
}
                if(!true) {
                    for(int i=0;i<n;i++) {
    ...
    ll ans = 1, cnt=1, mina = a[0];
    for(int i=0;i<n;i++) {
        if(i==n-1 || a[i+1]>mina+d) { // a[i+1]>a[i]+d, also 0%
...
正解：从小到大插入，排列顺序不影响方案数。排序后，最新要插入的设为 a[i]，对之前 j<i 的所有 a[j]，只要 a[j], a[i] 差距在 d 以内，这些 j 都可以选择。双指针/二分都可以找到 j。或者更简单的，直接做值域前缀和。计算 [j, i) 之间的方案数。每一步乘起来即可，方案数为组合数。
...
> ## 题目描述
>
> 工匠手里有一段由 $N$ 节小金块连接而成的长条金块，这些小金块从左到右依次排列，第 $i$ 节小金块的价值为 $A_i$。
>
...
> 第一行一个整数 $N$，表示小金块的节数。
>
> 第二行 $N$ 个整数 $A_1,A_2,\cdots,A_N$，用空格分隔，分别表示每一节小金块的价值。
>
> $$
> 1 \le N \le 10^5,\qquad 1 \le A_i \le 10^8
> $$
>
> ## 输出描述
>
> 输出一行一个整数，表示四段金块价值中最大值与最小值间可能的最小差。

赛时 WA 代码，感觉把前半段也用后半段的办法优化，然后枚举中间第二刀会好些，但还是 TLE

赛时思路是对的，贪心也就是局部最优，取2段的 max-min 最小化，一定不会让总答案更差，因为总答案是 max(s1, s2, s3, s4) - min(s1, s2, s3, s4)，其中 s1,s2 变化不干涉 s3, s4，只是优化了 s1, s2 的最大值变小，最小值变大
...
int main() {
    ...
    // j 是中间分割点，左右两侧都至少需要分成两个非空段
    for (int j = 2; j <= n - 2; ++j) {
        vector<int> leftCandidates;
        vector<int> rightCandidates;

        // 寻找 s[i] 最接近 s[j] / 2，i ∈ [1, j-1]
        int64 leftTarget = (s[j] + 1) / 2;
        int i = lower_bound(
                    s.begin() + 1,
                    s.begin() + j,
                    leftTarget
        ...
        if (i <= j - 1) leftCandidates.push_back(i);
        if (i - 1 >= 1) leftCandidates.push_back(i - 1);

        // 寻找 s[k] 最接近 (s[n] + s[j]) / 2，k ∈ [j+1, n-1]
        int64 rightTarget = (s[n] + s[j] + 1) / 2;
        int k = lower_bound(
                    s.begin() + j + 1,
                    s.begin() + n,
                    rightTarget
        ...
        if (k <= n - 1) rightCandidates.push_back(k);
        if (k - 1 >= j + 1) rightCandidates.push_back(k - 1);

        for (int x : leftCandidates) {
            for (int y : rightCandidates) {
                int64 p = s[x];
                int64 q = s[j] - s[x];
                int64 r = s[y] - s[j];
                int64 t = s[n] - s[y];

                int64 maximum = max({p, q, r, t});
                int64 minimum = min({p, q, r, t});
...
>
> 设一对匹配括号的位置为 $l, r (1 \leq l < r \leq n)$。这对括号的内部长度定义为 $r - l - 1$，即两个括号之间的字符数量。请计算内部长度能被 $k$ 整除的匹配对数量。特别地，0 能被任意正整数整除。
>
> 【名词解释】
> 合法括号字符串满足：从左到右扫描任意前缀时，左括号数量不少于右括号数量；扫描完整个字符串后，两种括号数量相等。
>
> 输入描述
> 第一行输入两个整数 $n$ 和 $k (2 \leq n \leq 2 \times 10^5; 1 \leq k \leq n)$，分别表示字符串长度、整除条件中的正整数。
...
> $$
> f(k) = \sum_{i=0}^{n-1} ((i \bmod k) \cdot a_i)
> $$
...
对于这道题，我的思路是这样的，你看一下有什么地方不对？ 首先使用根号分治，对于小于等于根号n的区域直接暴力计算。复杂度为n根号n。 然后使用整除分块，倒序计算， [n/1, n/1] 自成一块，先算， [n/2, n/1) 是第二块区间，从区间从右往左遍历k，对区间 [n/b, n/(b-1) ) 首先，令 f[k] = f[k+1]， 每块长度是k(最后一块按n截断)，遍历每一块，第i块，减去上一块的单点差，然后用a的前缀和给这一点下一个区间全部权重+1，一直遍历结束为止，

错误地方：没有考虑到第二块往后退2格、第三块往后退3格，忘记了。
...
$$
i\bmod k=i-k\left\lfloor\frac{i}{k}\right\rfloor
$$

可得

$$
f(k)=\sum_{i=0}^{n-1}i a_i
-k\sum_{i=0}^{n-1}\left\lfloor\frac{i}{k}\right\rfloor a_i.
$$

记

$$
W=\sum_{i=0}^{n-1}i a_i.
$$
...
=\sum_{\substack{m=k,2k,3k,\dots\\m\le i}}1.
$$
...
$$
\begin{aligned}
\sum_{i=0}^{n-1}\left\lfloor\frac{i}{k}\right\rfloor a_i
&=\sum_{\substack{m=k,2k,3k,\dots\\m<n}}
\sum_{i=m}^{n-1}a_i.
\end{aligned}
$$
...
$$
s_m=\sum_{i=m}^{n-1}a_i,
$$

则

$$
f(k)=W-k\sum_{m=k,2k,3k,\dots<n}s_m.
$$

对于每个 $k$，只需枚举它的倍数。总枚举次数为

$$
\sum_{k=1}^{n}\left\lfloor\frac{n-1}{k}\right\rfloor=O(n\log n).
$$
...
int main() {
    ...
    while (T--) {
        ...
        for (int i = n - 1; i >= 0; --i) {
            ...
            weightedSum =
                (weightedSum + static_cast<int64>(i) * (a[i] % MOD)) % MOD;
        }

        for (int k = 1; k <= n; ++k) {
            int64 floorSum = 0;

            // floorSum = sum floor(i / k) * a[i]
            for (int multiple = k; multiple < n; multiple += k) {
                floorSum += suffix[multiple];
                if (floorSum >= MOD) floorSum -= MOD;
            }

            int64 answer =
                (weightedSum - static_cast<int64>(k) * floorSum) % MOD;
...
第一块不处理；第二块先全体+1，第一项再-5。第三块全体+2，前两项-5，以此类推。
...
int main() {
    ...
    while (T--) {
        ...
        // preA[i]  = sum_{j=0}^{i-1} a[j]
        // preIA[i] = sum_{j=0}^{i-1} j * a[j]
        vector<int64> preA(n + 1, 0);
        vector<int64> preIA(n + 1, 0);

        for (int i = 0; i < n; ++i) {
            cin >> a[i];
            a[i] %= MOD;

            preA[i + 1] = (preA[i] + a[i]) % MOD;
            preIA[i + 1] =
                (preIA[i] + 1LL * i * a[i]) % MOD;
        }

        auto rangeA = [&](int l, int r) -> int64 {
            int64 result = preA[r] - preA[l];
            if (result < 0) {
                result += MOD;
            }
            return result;
        ...
        // 不优化就不需要
        auto rangeIA = [&](int l, int r) -> int64 {
            int64 result = preIA[r] - preIA[l];
            if (result < 0) {
                result += MOD;
            }
            return result;
        ...
        for (int k = 1; k <= B; ++k) {
            ...
            for (int l = 0; l < n; l += k) {
                ...
                int64 sumA = rangeA(l, r);
                int64 sumIA = rangeIA(l, r);

                int64 contribution =
                    (sumIA - 1LL * l * sumA) % MOD;
        ...
        for (int k = n - 1; k > B; --k) {
            ...
            for (int q = 1; q <= maxQ; ++q) {
                 ...
                 * 整个第 q 块权值增加 q。
                 */
                int64 wholeBlock = rangeA(l, r);

                delta += 1LL * q * wholeBlock % MOD;
                if (delta >= MOD) {
                    delta -= MOD;
                }
                 ...
                 * 需要额外减去 k+1。
                 */
                int middle = min(r, q * (k + 1));

                if (l < middle) {
                    int64 crossedPart = rangeA(l, middle);

                    delta -= 1LL * (k + 1) * crossedPart % MOD;
                    if (delta < 0) {
                        delta += MOD;
                    }
                }
