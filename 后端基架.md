...
```mysql
CREATE DATABASE IF NOT EXISTS 数据库名 DEFAULT CHARSET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
```

`utf8_general_ci` 数据库的校验规则,ci是case insensitive的缩写,意思是大小写不敏感;相对的是cs,即case sensitive,大小写敏感。ci的话，外键和查询都可以不区分大小写。

> `ai`*/*`as` = accent insensitive / accent sensitive（是否区分重音）：`e`、`é`、`è` 在比较时会被当作“同一类”来处理。
...
不支持 INTERSECT, MINUS, EXCEPT，但其他数据库有支持的。
...
```sql
mysqldump -u username -p --no-data 数据库名 > all_tables.sql
```

这会生成一个包含所有表结构但不包含数据的SQL文件。
...
如：(本地的话 `-h 127.0.0.1` 可以不写)，下例 test 是数据库名，如若不写，则可根据 sql 的 (create database +) use 来确定，use 会覆盖
...
`least(a, b, ...)` `greater(a, b, ...)` 返回最小和最大参数(字符串也行)
...
> 牛客：求字符 `*` 出现次数：`SELECT text_id,LENGTH(text_content) - LENGTH(REPLACE(text_content,'*','')) AS num FROM textTable;`
...
gitpod / docker / vscode (wsl+remote ssh)

##### debug模式

在 `build.sh`，找到参数。清理目录 `build_debug/`。 运行 `bash build.sh debug`。多线程编译，该目录下：`make -j8`。然后在 `build_debug/bin` 下看到 `observer`

运行，build_debug 下，`./bin/observer -P cli`。可以打断点。可以用 vscode 的 debug。debug 模式编译不会优化，release 可能会有划掉导致有的看不到。

debug (vscode) 的监视功能可以添加对某个变量的监控，此功能也可以监视某个地址，先看变量地址。可以逐行执行(逐过程F10)，也可以跳进函数去执行(单步调试F11)，也可以跳出。跳到下一个断点(F5)。可以看调用堆栈来分析过程。可以方便快速学习代码。（GDB）

日志调试，可能需要反复重新编译，比较麻烦。但多线程协调关系，可以通过日志调试。如 LOG_INFO, LOG_WARN 方法(以 `src/observer/storage/table/table.cpp` 为例)，注意日志级别，用来筛选日志，以免输出一大堆不相关的东西。快速筛选的技巧：日志内容添加一下便于搜索的关键字，方便增删这条日志。也可以调参数来筛选哪些可见。

ASAN工具。编译选项，默认开启，ENABLE_ASAN，在 `CMakeLists.txt` 里。那么可以爆错输出看栈的时候可以看到对应是在哪里开始内存越界、泄露、野指针之类的问题的。

单元测试。知道出问题，但不知道根源在哪里。按照模块来排查。逻辑复杂模块，写完之后，配套做。根目录下 `unittest` 目录，依葫芦画瓢，使用谷歌 gtest 框架。会自动检测出来的。在 `build_debug/bin` 里得到的东西，cd 进去执行即可。

SQL DEBUG。在 cli 里，开启 `set sql_debug=1`。代码里添加，就执行 `sql_debug` 函数，参考如 `src/observer/sql/operator/table_scan_physical_operator.cpp`。也可以在 `src/observer/session/session.h` 的 `bool sql_debug_` 设置为 true。

#### 模块简介

> OceanBase 的教学用数据库，[官网](https://oceanbase.github.io/miniob/)，C++
...
在 `src/observer/net/Communicator.h` 可以看到 session 等代码，存储会话相关的变量。

几个通讯协议，如 PLAIN，SQL 封装为 + \0。

##### 线程模型

`src/observer/net/thread_handler`。

朴素：一个线程对应一个客户端请求。`one_thread_per_connection_thread_handler`

线程池：客户端发送的请求放到队列，线程处理完就发回给客户端。`java_thread_pool_thread_handler`

> 还有可能需要关注的 `sql_task_handler.h`，处理 SQL 各个阶段。

##### parser

`src/observer/sql/parser`

SQL 命令解析为语法树，LEX(flex)做词法解析，YACC(bison)做语法解析，生成描述性数据结构。也就是分词+建树

![_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=7680115676775277155&skey=@crypt_66a6c333_8bb675d1123cb20149196636e6a319f2&mmweb_appid=wx_webfilehelper](img/_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=7680115676775277155&skey=@crypt_66a6c333_8bb675d1123cb20149196636e6a319f2&mmweb_appid=wx_webfilehelper.jpg)

词法分析 `lex_sql.l`，语法分析 `yacc_sql.y`，修改的话，改完用 `gen_parser.sh` 生成

##### transformer/rewriter

``src/observer/sql/optimizer`

在确保执行结果相同的条件下做等价变换，多种改写可以相互触发。

如：视图合并、连接条件下压、子查询展开、外连接消除、等价类传递、limit下压、谓词转移、或展开、窗口函数转化、公共表达式提取。

> 举例：最简单的，如把 `where 1` 去掉之类的。`rewriter.h`

语法语义前端 -> (基于规则的改写 <-> 基于代价的改写) <-> 物理优化 -> 执行。

##### optimizer

(是否用索引，之类的，根据统计信息和规则判断，如 IO, CPU 开销)

路径选择：主键/二级索引/全表遍历。 

代价模型：System R 模式基于代价的优化器

连接算法：左深树/右深树/多枝树，nested loop join / merge join / hash join

##### executor

按照语法树/执行计划，访问元数据和存储引擎，计算出最终效果，如

```mermaid
graph LR
	a("PRODUCT<br/>a.SUM(b+1)")-->b("GROUP BY<br/>a")
	b---c["AGGREGATOR<br/>SUM(b+1)"]
	b-->d("PREDICATE<br/>b>100 and a>10")
	d-->e("TABLE SCAN<br/>tab")
```

数据库火山模型。迭代的方式，算子调用别的算子做运算。

##### buffer pool

`src/observer/storage/buffer`

磁盘数据与内存缓存。兼顾访问效率与内存使用量。

每个文件按等大的页划分，每次读页，内存帧对应一个或多个页；做帧淘汰的 LRU 等算法

每个文件 page0 是特殊页，描述页数、哪些页分配和空闲、和 01 表。见 `disk_buffer_pool.h`，可以找到 `BPFrameManager, DiskBufferPool`。

##### record manager

`src/observer/storage/record_manager`

组织记录在磁盘上的存放，即处理记录的增删查，高效利用磁盘空间，减少空洞；通过 buffer pool 与磁盘交互，记录存在页中；支持定长记录

页头+槽数组+具体记录 (定长，所以槽就是 01 表，可以定位哪些记录位置有效)

#### 存储结构

##### 概述

一个文件夹就是一个数据库，下面有数据表元数据 `表名.table`，数据 `表名.data`，`表名-索引名.index` 文件。

第一个页用来页头，组成：页号、页数、01表，因为 01 表的长度有限，决定了最大总页数固定。普通页构成：页号、数据。

记录管理。记录设置都是短的。可以看 `purge_page, dispose_page, allocate_page` 等函数，进行页面的增删改查。`record_manager.h`

可以在 `/src/observer/storage/index` 找到 B+ 树，`storage/record` 找到记录。
...
- LeafIndexNode，有：
  1. page_num 页序号
  2. common header (IndexNode) is_leaf, key_num, parent (page num)
  3. prev_brother, next_brother (page num)
  4. data (key-value 顺序对), key 是索引列+RID(该行数据在磁盘的位置)， value: RID
- InternalIndexNode
  - 少了上面第三行的兄弟指针；value 不存 RID 而是 page num
  - 第一个 key 无效，不用于比较
- page 存在 IndexFile 里，page0 是 IndexFileHeader，元数据：
  - root page
  - internal max size (internal page 最多的键值对数目)
  - leaf max size (leaf page 的)
  - attr length 索引列长度
  - key_length 索引列长度+RID长度
  - attr_type 索引列类型，如 INT
  - pad 无效填充

冗余的 key-value 存储有利于分裂合并更加方便

#### 词法语法解析

##### 词法

`src/observer/sql/parser/lex_sql.l`，flex  (Fast Lexical Analyzer)。

1. top{}里的是与`lex_sql.cpp` 的，
2. 第二部分是 `%{}` 的，也是一样的。之后是模式匹配。
3. 首先是选项，如 `%option`。其中 `%option bison-bridge` 是与语法分析适配。`%option reentrant` 是可重用，即，一个函数或代码段能够在执行过程中被中断，然后在中断服务程序中再次被安全调用，而不会导致数据损坏或逻辑错误，故允许多线程。还有大小写敏感等。
4. 定义，如 WHILE_SPACE 是哪个正则表达式
5. %% 包括住的地方，是token对照表，如字符串、正则表达式(如浮点数)转token。可以用大括号引用定义。其中，`{ID}` 是字符串去掉引号后的内容
6. 最后一部分，C++ 代码

规则：最长的优先，如 TABLES 比 TABLE 先匹配。同长，谁写在代码前面谁先匹配。需要安装 flex。每次变更了语法都需要手动执行 `parser/gen_parser.sh`，生成同名 .cpp。

##### 语法

`parser/yacc_sql.y`, (Yet Another Compiler Compiler)，其开源替代是 bison。
...
4. %union {}，联合体，是每一个匹配结果的数据类型(属于里边的某一个)；随后用 %type 定义解析结果的数据类型，即 `%type <结果类型> 语法规则`，表示该规则生成什么类型。最后一条表示最终的整体结果 (commands) 是 sql_node

5. %% 包住的内容。语法规则可以层次定义。如定义 `commands:`。然后定义 `command_wrapper:` 是一堆命令。如其中，`exit_stmt` 是 EXIT 字符串 + 格式语法结构。在 `parse_defs.h` 可以看到。如再看 `select_stmt`。如只支持属性现在是。它里边写着的 select_attr，又是一个定义的东西，如是 `*`，是一个或多个字段(rel_attr / attr_list，继续定义)，其中递归定义的 attr_list 是 `COMMA rel_attr attr_list`。

   $$ 是返回值，$2 是第二个 token，delete释放掉。其中，calc 实现不完全。

6. 最后，函数入口

`src/observer/sql/expr/` 里的 `value.h`, `expression.h` 等定义了相关具体内容。vscode 终端有 run task 可以用 gen_parser，生成 cpp 文件。

#### 实现示例

##### droptable

`src/observer/storage/db/dbb.cpp`。编译，运行。查看报错日志：如 `build_debug` 的 `observer.log.yyyymmdd`

在 `src/observer/sql/stmt/stmt.cpp` 添加 case 指令，添加文件 `drop_table_stmt.h` 和 `.cpp`。改 create table 的代码。构造函数，删掉不必要的参数。把 `CreateTableSqlNode` 改了。对应看着删删。

在 `command_executor.cpp` 添加 case。再加一个 `drop_table_executor.h` 和 `.cpp`。同理改 create table 的代码。table 用 find_table 找。其他的对着改改。

看到 `src/observer/storage/table/table.h`，实现一个 drop 函数。释放掉对应的资源。有 bufferpool, record handler 等，索引的。比如对应找找 `storage/buffer/disk_buffer_pool.cpp`，补一个 `remove_file`，关闭文件等操作。`Index.h` 添加 destroy 函数。然后 `storage/index/bplus_tree.cpp` 也要添加 destroy 函数，参考关闭文件来写，

##### date

可以看 `test/case/test/primary-date.test`。可以存储为字符串，或int(如高4位年，中2月，后2日)。对 `src/observer/sql/parser/lex_sql.l` 添加 DATE (仿照 CHAR)，`yacc_sql.y` 的 token 也加。且 type: 的定义或加加上去 DATE_T。那么 `src/observer/common/type/attr_type.h` 的 AttrType enum 要加 DATES。其 `.cpp` 的 ATTR_TYPE_NAME 要加枚举字段 dates 字符串。更新语法。

继承 `type/data_type.h` 基类。要做的运算基本都实现一下。若用 int 实现，很多东西可以复用 int 的。注意 `data_type.cpp` 的 array 要添加 DateType make unique。compare_int 参数转 void* 指针(即 `(void*)&x`)。修改 `cast_cost` 和 `cast_to`，对 `char_type.cpp`，使得可以从字符串转 DATE。记得做一下合法检验，如闰年月日不对和其他非法。`value.h` 加一个 set_date 方法。其 `.cpp` 的 set_data 类加一个 switch case。
...
兼容目标mysql5.7，绝大部分支持。还有 API 接口和可以迁移到数据仓库如 hadoop 等。

##### 架构

对标 mysql 的话：

1. 查询缓存：fast-parser, plan cache
2. 分析器：parser, resolver
3. 优化器：transformer, optimizer
4. 执行器：code generator, executor

流程：

```mermaid
graph LR
SQL请求-->11["Fase-Parser"]
11-->12["Plan Cache"]
12-->Parser
Parser-->Resolver
Resolver-->Optimizer
Optimizer-->41["Code Generator"]
41-->Executor
12-->Executor
41-->12
```
...
扩展性：垂直扩展(单机硬件性能提升)、水平扩展(节点增加)

对等节点：无共享、OBServer 包含 SQL、存储、事务

> 无共享：每个节点独立拥有自己的资源（CPU、内存、存储）、不共享内存或磁盘存储、通信仅通过消息传递进行。
>
> 对等节点：所有节点在功能上是等同的、没有中心控制节点或主从关系。
...
双中心：主备库。物理复制等(CLOG commit log)。还有双地三中心。三地五中心。
...
Patition group (PG) 包括多个 partition (分区)；每个 partition 包含多个 table store。每个 table store 就是 LSTM-Tree 架构。PG 是一个物理节点。

内存里，memtable 用 Btree 和 hashtable 组成，存储的是指向数据的指针。提交版本通过回调函数获得。

- 哈希表适合按主键查找，DML 时主键校验等。B 树适合范围查找

##### LSM-Tree

与传统架构类似。磁盘划分为三层，L0 层 tiered 模式，即允许有多个 mini SSTable。L1 层一个 minor SSTable，L2 层一个 major SSTable。

三个压缩：

1. Mini Compaction

   Frozen Memtable 转 Mini SSTable。压缩结束后释放 Frozen 和数据 log，是 tiered 类型的 compaction。

2. Minor Compaction

   L0, L1 层 SSTable，

   多个 mini SSTable 合成一个 (L0 -> L0 tiered)，mini 少选这个

   或 多 mini 和一个 minor 合一个 minor(L0 -> L1 leveled)。mini 与 minor 相近选

   自动触发，超过阈值就触发。减少 RA 问题，减少 SSTable 数量。

3. Major Compaction

   整个集群选择统一的 major 位点，每个分区使用该 major 位点坐快照查询，将查询结果持久化生成 major SSTable。使得 L1, L2 全部结果合为一个 major SSTable。触发方式：

   1. 自动触发。mini compaction 执行次数达到一定阈值时。以缩减 SSTable 数量，降低 RA。
   2. 定时触发。避开业务高峰期，设置触发时间，在业务低峰期定时触发合并。
   3. 手动触发。命令。




每个 SSTable 由若干 2MB 大小的 Macro Block 组成，每个 Macro Block 有若干个 Micro Block。

1. 全量合并：类似 HBase, Rocksdb 的 Major Compaction。把当前 Major SSTable, Mini, Minor 所有的合到一起，成为新的 Major SSTable。

2. 增量合并：只重写发生了修改的宏块/微块，未修改的直接放入新的 Major SSTable。省去了解析行，编码，计算列 checksum 等操作，改善 WA 问题。

3. 渐进合并：DDL 如增删列，修改压缩算法，需要重写全部数据。重写延迟到合并，使得增量回退到全量。渐进的核心是数据重写分散到多次合并去做，每次只重写一部分数据。如一次合并重写 1/60 的数据。

   ```sql
   alter table t1 set progressive_merge_sum = 60;
   ```

4. 轮转合并。规避合并对业务的影响。多个部分轮流执行合并，用户操作的流量导向没执行合并的副本。

5. 并行合并。将需要完成的 rowkey 值域划分为若干个子任务并发。默认开启，后台对数量大的自动做。

##### SSTable

含多个 Macro block。每个 2MB 定长，是数据写 IO 的基本单位，其组成：

1. Macro block header (用于定位，查找)
2. 若干 micro block (读 IO 的基本单位，变长)，两种存储格式：
   1. 行存：flat format + 若干 row
   2. 列存：encoding format (提高数据压缩率)，若干 col
3. micro block index (偏移，key 范围等)

##### get查询

get 查询流程：

- fuse row cache 如果不是最新或不命中，就要去查

![_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=4154893084914662275&skey=@crypt_66a6c333_fab3a1d3aa1baf34c56b6e90d0af931c&mmweb_appid=wx_webfilehelper](img/_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=4154893084914662275&skey=@crypt_66a6c333_fab3a1d3aa1baf34c56b6e90d0af931c&mmweb_appid=wx_webfilehelper-1751265768785-3.jpg)



get 查询 (查不到 memtable 时)

![_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=8657037013035982227&skey=@crypt_66a6c333_fab3a1d3aa1baf34c56b6e90d0af931c&mmweb_appid=wx_webfilehelper](img/_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=8657037013035982227&skey=@crypt_66a6c333_fab3a1d3aa1baf34c56b6e90d0af931c&mmweb_appid=wx_webfilehelper.jpg)

查 SSTable 遇到同一个 rowkey 有多个记录时，按照时间合并(归并排序)。

##### scan查询

scan 查询：不知道主键，也就是需要遍历去找到符合 where 要求的。由于每个都要看，所以要遍历。

二分找到宏块ID，然后再索引找到微块信息，去扫描。

为了简化多路归并，使用了败者树(loser tree)，多路归并排序的高效数据结构，是堆排序的一种变体。

- 每个非叶子节点记录败者(比较的较大值)，胜者继续向上比较，根记录当前全局最小值。它比最小堆比较次数更小，重构过程更高效，适合外部排序和大规模数据归并。

![_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=1716565588363974939&skey=@crypt_66a6c333_fab3a1d3aa1baf34c56b6e90d0af931c&mmweb_appid=wx_webfilehelper](img/_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=1716565588363974939&skey=@crypt_66a6c333_fab3a1d3aa1baf34c56b6e90d0af931c&mmweb_appid=wx_webfilehelper.jpg)

查询优化：过滤条件下压(提前做过滤)、延迟物化、向量化(CPU流水线，过滤时顺便做)等。

异步，避免阻塞流程。查询预热(提前加载热点数据)。

> ##### 索引回表查询

回表：索引local index table里scan，然后对应去main table里get查询。
...
基于代价模型、统计信息、查询改写(基于规则+基于代价)，先后进行基表访问路径选择、连接顺序和连接算法、其他算子分配，最后给到执行器。

查询改写：SQL 转化为易于优化(如可选操作变多)的形式。

挑战：正确性、完备性(多种写法都要能)。

模式匹配(保证完整性)、等价变化(正确性)、有效性判断(基于规则、代价)

1. 基于规则：总是把 SQL 往好的方向改写

   > 例如把 in 子查询改写为 join，使得可以不同的 join 实现

   如支持恒正/恒假条件优化、常量折叠、SPJ和非SPJ的视图合并、连接消除、非聚合类子查询提升、外连接消除、聚合类子查询提升、谓词推导、limit-k下压、roup by/window func/distinct/order by消除。

2. 基于代价：需要计算代价才能决定是否把 SQL 往好的方向改写

   overhead 重(时间+空间)，完全基于代价未必是好选择，可能会造成回退

   > 如估算 or 改写为 union all 的代价；为了不重复，a or b 等价于查询a union all (b and not a)，若有索引，这个改写可能会更好。而且不一定能算的准确

   如or-expansion, group by replacement, 窗口函数改写、semi to inner 改写、子查询合并

OceanBase 是不断同时迭代(规则->代价)直到收敛

##### 物理优化器

计划枚举：

1. 计划形状：left/right deep tree, deep tree, bushy tree 等

   - left deep 每个节点（连接操作）的 左子节点 要么是一个基表，要么是另一个连接操作的结果；而右子节点 总是一个基表 `A ⋈ B ⋈ C ⋈ D`
   - deep 树的结构是线性的（linear）或链状的（chain-like），即任何连接节点之下，至少有一个子节点是基表
   - bushy tree 树中的一个或多个节点（连接操作）的两个子节点 都不是基表，而是其他连接操作的结果（即它们本身也是子树）`(A ⋈ B) ⋈ (C ⋈ D)`

   一般不用 bushy，效率不高。一般是 deep。

2. 枚举算法：DP / linear algorithm (如>10个表回退线性，启发式 / 贪心)

代价计算：

1. 统计信息：基础统计信息+直方图
2. 选择率计算和中间结果估算
3. 代价模型：IO+CPU+NETWORK

例如，两表连接，考虑 hash join, merge join, next loop；在左在右分别试一下，一共 2x3=6。

对星型查询，表个数增加，等价逻辑指数增长。

##### 基表访问路径生成

枚举所有只有1个表的执行计划(基表路基选择生成，即做主表还是索引)

1. 前置规则：确定性，某索引满足规则直接选它(如唯一索引覆盖，不用回表)

2. 若前置规则无法命中，选skyline剪枝。对两个评价指标，多个方案构成的散点，为了权衡指标，从左下到右上的凸包点集连成的线就是skyline。

   只选择这条线上的点，达到剪枝的效果。三个维度考虑：是否回表、是否存在 intersting order (某个索引的排序在查询中被使用，就是存在；即用索引来化简 order by)，索引前缀能否抽取 query range (简单理解为是否能快速定位数据)。全体点集是各种索引
...
递推转移方程里枚举所有的连接算法，交换左右顺序。做完之后，接下来：

维护的计划集合：代价最小的计划、有 interesting order 的计划。

继续分配其他算子，如 group by (分别尝试哈希，group by)，维护更新这个计划集合；对窗口函数，order by, limit 同理。最后求出代价最小的。

这就是单机非分表的查询的大致过程。

#### 执行引擎

架构：APP 发送请求给 OBProxy 后，尽量让主副本执行。3个分区，一个主副本，2个备副本。

##### 单行模型

第二代设计是单行执行，火山模型。

1. 强类型计算。与第一代弱类型的相比，弱类型把编译器要做的事情放到了执行器，加大负担。
2. 内存与分配。编译期确定执行过程内存分布，执行前预先分配所需内存块，执行时重复使用。分到 data_frame。
3. 数据按行迭代，表达式Expr列表描述行Array<Expr*>
4. 每个表达式对应一个Datum

缺点：按行迭代和计算，迭代次数多。虚函数调用开销大。对CPU cache不友好

##### 向量化

改进，第三代：向量化模型 (算子向量化)

按 batch 迭代和计算，算子之间，算子内部都是按 batch 实现。

好处：迭代次数少；更少虚函数调用，CPU cache 更友好。易于 SIMD（Single Instruction, Multiple Data，一种利用处理器并行处理能力的性能优化技术。它通过单条指令同时处理多个数据）优化。方便利用硬件特性，比如DDR预取。



内存布局：数据按列形式组织。也是更好优化。局部，连续，cache 友好，反复使用可以，可以 SIMD 优化。

存储向量化：

1. 微块内数据直接案列存放
2. 存储按列直接投影到SQL引擎
3. 存储层filter向量化过滤，减少开销
4. 行列混存架构
...
1. 执行计划被划分为多个DFO(data flow operator)，每个DFO内部也可以多线程
2. 两级调度，父子 DFO 同时调度(不写中间结果)
3. DFO 之间通过 DTL(data transfer layer)流式传输数据
4. 支持多种数据重分布方式

![_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=6009296444597358117&skey=@crypt_66a6c333_6699c149e9a38360d5cafbac4ab32f7c&mmweb_appid=wx_webfilehelper](img/_cgi-bin_mmwebwx-bin_webwxgetmsgimg__&MsgID=6009296444597358117&skey=@crypt_66a6c333_6699c149e9a38360d5cafbac4ab32f7c&mmweb_appid=wx_webfilehelper.jpg)
...
1. Hash(join/group by/distinct)

   操作方式：按join键/group by列/distinct列计算哈希值，再根据worker数量取模，将相同哈希值的行分配到同一个worker处理。

2. Broadcast(join)

   操作方式：将左表的全部数据广播到所有worker节点，每个worker独立完成与右表的join操作。

3. Broadcast2Host(nlj)

   操作方式：针对内嵌循环连接（inner nested loop join），将左表数据广播到右表所在的机器，右表的所有worker协同执行nested loop join。

4. Pkey(join)

   操作方式：左表根据右表的分区键进行数据分发，确保属于同一分区的行被发送到同一个worker处理。

##### 执行模式

1. 本地local执行

   数据都在接受请求的节点；无并行，不需访问一个表的多个分区

2. 远程remote

   SQL要访问的数据都在非接受请求节点，同一个节点里；无并行，不需访问一个表的多个分区。执行计划或SQL发送给有数据的OBServer，其返回结果数据。

3. DAS(data access service)

   数据在多个非请求节点，处理分布式OLTP查询，通过RPC访问操作远程数据，即先从一个节点拿数据，再把数据给到另个节点执行
...
```mermaid 
graph TD
ty{语句类型}-->|非INSERT语句|数据过滤
ty-->|INSERT语句|主键冲突等查询
ty-->|CMD语句|开始提交事务
数据过滤-->lock{是否需要加锁}
主键冲突等查询-->insert["Btree/Hash中插入一行"]
insert-->加行锁
lock-->|是|加行锁
加行锁-->crud["数据查询/修改"]
lock-->|否|crud
开始提交事务-->commit["2PC/1PC/ELR事务提交"]
commit-->suc{事务提交成功?}
suc-->|否|commit
crud-->endd["返回SQL继续执行/应答客户端"]
suc-->|是|endd
```
...
快照是数据在某一时间点的映像。时间点作为版本号记录下来。可以用于数据恢复(事务回滚)、并发控制。修改记录，包含数据、版本号、事务信息。

读取，能读到<它版本号的最大的写信息。

设事务T1,T2修改了a，T3读取a；T1提交了，T2还没结束。T3会等待T2结束，若T3 read 版本号比 T2 的提交版本大，返回 T2 结果；否则，返回 T1 结果。

可能T2要很久，所以事务可以申请prepare version(小于commit版本)。若T3的读版本比T2的准备版本还要小，那肯定比commit小，直接返回T1结果；否则，继续等待T2提交。

初始prepare版本设置为0，使得如果还没进入准备阶段，必然需要等它。

总结：

- read snapshot (语句快照，读<=该版本的最新快照)
- prepare version：2PC 中参与者 prepare log 时间戳
- commit version：事务提交时间戳，所有参与者都是它
- local timestamp service LTS：本地时钟服务，1.x 版本，性能比 GTS 好
- global timestamp service GTS：全局一致时钟服务，保证事务外部一致性 2/3.x

##### 2PC

具体而言，当使用版本号时，传统：

1. 确定提交版本号为所有参与者prepare version的最大值为事务commit version
2. 持久化redo log, commit log
3. 应答客户端

响应时延：4次日志延迟+2次RPC延迟。

希望：快速提交(应答)、快速恢复、更小数据量。

优化：

1. 协调者无需写日记(事务开始+请求准备，准备完成+提交请求)，因为可以从参与者得知
2. 无需第二阶段等参与者写完commit log再通知客户端。哪怕宕机了，直接redo prepare的即可，第二阶段对可恢复不是必要的

优化为1次日志延迟+1次RPC延迟。

但也产生新问题：外部一致性问题。版本不一致。使得查询可能查不到对的版本。

#### 内存管理

##### 线程

系统线程：请求、响应 client 请求的网络线程，调度转储任务、执行转储任务等

工作线程：执行 SQL/Trans 的核心流程

数据流转：

- 网络线程处理响应 client 请求，将请求发送到用户的请求队列  
- 用户工作线程消费队列中请求，执行 SQL/Trans 的核心流程  
- 事务提交过程中提交本地日志时先写入内存 IOBuf 中，等待 Clog 线程完成落盘  
- 事务提交过程中的 RPC 消息通过网络线程发出  
...
每个工作线程上保留2M大小的buffer pool，专用于SQL处理过程中的临时内存需求

•  高效。由于仅限本线程使用，整个链路上可完全无锁
•  零碎片。SQL处理过程通常要连续申请大量小内存块，由于总是优先从线程上申请，这些内存块与其他模块的长生命周期内存完全隔离，完全避免了可能的碎片问题
•  零内存泄漏。由于专用于处理SQL请求，SQL结束后可完全回收内存，即使代码存在泄露BUG也能兜底

使用 KVcache。连续分配。内存泄漏在线诊断，泄露发生时，发生后都能解决。

安全漏洞自检：double free, buffer overflow。

不使用 addresssanity，因为性能不佳。

使用 clang 编译插件，指令插桩。如 llvm pass 写编译插件。

use-after-free (UAF)，隔离区 (quarantine)，释放内存时，先将其放入一个区域(如队列)，等一段时间再释放，延迟内存复用的时间，提高 UAF 问题的能力。优化 callstack 就地记录内存块上，无索引开销，使用 libunwind 库，比 qlibc 好很多。

double free：重复 free 时，检查 shalow 字节(每个位代表某个字节是否可用) 触发 coredump。

UAF：复用前 shadow 字节检测触发 coredump，复用后内存重置，不可阻挡
...
- 代码规模庞大：300万行核心代码
- 系统复杂度高：需保障高可用性、扩展性和数据一致性
- 功能模块复杂：功能模块多且逻辑极其复杂
- 耦合度高：模块间相互耦合，牵一发动全身

OceanBase质量体系贯穿研发全流程的各个阶段，确保版本质量。

- 需求收集：客户需求、TL(技术领导?)技术需求
- 需求决策：隔周新需求、讨论
- 研发测试：系分/测分、编码、测试
- 版本发布：release、报告
- 触达客户：试用、Poc、上线
...
1. 规划阶段：新需求风险识别
2. 开发阶段：方案评审、遵守编码规范、单测/自测/联测、CODEOWNER制、准入测试
3. 测试阶段：SQL功能测试、SQL随机测试、性能测试、系统稳定性测试、升级兼容测试
4. 准发布阶段：代码合入审批、链路组件适配、平台工具适配、BUG和风险管理
5. 客户/线上环境：版本管理、问题&风险同步

##### 质量体系

- SQL 功能测试：MysqlTest
- SQL 随机：RQG(Random Query Generator)、SQLancer
- 性能测试：Tpcc、Tpch、Sysbench
- 稳定性测试：Obopentest
- 兼容性测试：OBD、OCP

##### SQL功能测试

特点：功能多、形式多、新功能加入持续、模块耦合重。工具要求：新增case方便、快速迭代、可读性强、具有随机性。

MySQLTest没有随机性。Random Query Generator不快速可读性不强。二者结合，四个要求都能满足。

MySQLTest基于MTR(MySQL Test Run)二次开发。原理是执行结果与结果文件对比，类似OJ。case包含SQL、Command (控制运行时行为)、comment。其中command如shell指令，connect连接，eval执行SQL语句，sleep等大量命令。

case 存放位置，目录结构，编写规则等略。OBD 来运行。官网查看。

RQG是Mysql开发的，Perl实现。数据文件`.zz`自动生成关系表灌入数据（数据字段类型，范围等），语法文件`.yy`随机生成SQL语句。SQL里占位符替换为具体的表、数据等。可以递归定义。

##### 正确性测试

不正确的情况：写入数据不正确、查询结果不正确、多副本之间数据不一致、数据表和索引表中数据不一致。可以 MysqlTest (与预期作对比，很难完全覆盖)，RQG (需要两表完全一致，与其他表作对比)，数据库加内部校验(OB checksum自校验)。

SQLancer，基于 2020 三篇论文核心思想 PQS (pivoted query synthesis，用随机 true 表达式做查询), NoREC (优化器前后对比) , TLP (全表结果=某表达式为true结果 并 该表达式false结果 并 为null结果)。验证 SQL 逻辑正确性。
...
Sysbench 支持多线程基准测试工具，支持定制 lua，灵活性高。

TPC-C，最权威经典的 OLTP 标准，商品销售模型，9表5事务。实现有 tpcc-mysql, bmsql 等。OB用了后者。

TPC-H，最流行 OLTP。8表22条分析查询，模拟供应采购。

##### 稳定性测试

workload 和容灾组合。ChaosBlade 是阿里巴巴模拟cpu、磁盘、网络、IO、POD(Kubernetes中的最小调度单元)故障的工具，与 K8S 组合，实现了 Obopentest。

##### 兼容性测试

升级自动化，做复杂的测试，自动化操作，按zone滚动升级，升级期间不停服，对用户影响小。workload + OBD / OCP
...
建向量索引 (vsag: Vector Search Algorithm Library)，只支持 HNSF 

```sql
create vector index idx1 on t1(c2) with (distance=inner_product, type=hnsw, lib=vsag);
```
...
```sql
insert into t1 values(21, '[0.748479,0.276979,0.555195]'), 
                    (22, '[0.748479,0.276979,0.555195]'), 
                    (23, '[0.748479,0.276979,0.555195]');
```

ANN查询 (若不加 APPROXIMATE 就是精确索引)

```python
SELECT c1 from t1 order by l2_distance(c2, [0,0,0]) APPROXIMATE limit 1;
```
...
1. IVFFLAT，关系表存储中心节点和每个节点所属簇的映射
2. HNSW，关系表存储

好处是数据库兼容，如多版本控制，持久化等可以复用。但迭代器访问数据库表效率低，外存访问速度慢。

因此设计了内存向量搜索库 VSAG 库，增量 HNSW 内存索引 + 基线 HNSW 内存索引 + 维护记录可见性的内存 bitmap。做一个向量索引适配器。

OB 扩展的索引表结构：ACID、分布式存储。使用5张表实现。

会遇到的问题：

- new/malloc 内存管理与 OB 自主管理冲突。 memory hook / 修改库
- 持久化 HNSW 与 SSTable 冲突：认为用 blob 类型存储
- 多版本数据访问不支持：索引辅助表
- 支持在线插入，但在线删除破坏多层图结构连通性，难以 DML：索引辅助表
