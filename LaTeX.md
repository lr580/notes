...
需要引用文献时，用 `\cite{ascii名字}` 。注意大小写要保持一致。那么此处会出现 `[编号]`。可以带一个可选参数，表示引用编号。如 `参见\cite[page 13]{article1}的表述`。这样会以 `[编号, 可选参数]` 显示内容。
...
手动 footnote 办法 [参考](https://blog.csdn.net/weixin_41181312/article/details/124049703)

```tex
\footnotemark[1] % 出现上标
\footnotetext[1]{文字} % 出现本体
```
...
`align` 代码块可以进行对齐，使用 `&`。不加 `&` 自动右对齐。也会自动编号，不想的话使用 `\nonumber` 或 `\notag`。(amsmath 包)
...
\AtBeginSection[]{
    \begin{frame}
        ...
        % \addtocounter{framenumber}{-1} 当前页不增加页数
    \end{frame}
}
```
...
> ```tex
> \begin{frame}{Sample}
> The figure \ref{img_spn} shows an example SPN of $2$ boolean variables.
>     \begin{figure}[H]
>     \centering
>     \includegraphics[width=0.9\textwidth]{spn.png}
>     \caption{Sum Product Network}\label{img_spn}
>     \end{figure}
> \end{frame}
> ```
...
按 bib 格式和编译选项，会对一个参考文献的不同部分以不同颜色显示。

字体比较大，可能三篇论文就放不下了。可能要手动或 `thebibliography` 代码块，要么就缩小字体。

可以允许分页，和不计入页码(则不断重复上面的页码)
