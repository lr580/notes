...
求 $a^x\equiv b\pmod p$，设 $t=\lceil\sqrt p\rceil,0\le j\le t-1$，设 $x=it-j$，显然可以覆盖所有 $0\le x\le p$。原式为 $a^{it-j}\equiv b\pmod p$ 即 $(a^t)^i\equiv ba^j\pmod p$。预处理出 $a^t$，籍此计算出全部的 $ba^j$。通过哈希表，一次枚举 $a^{ti}$ 时可以直接查询其是否与此前任意一个 $ba^j$ 相等，相等则有解。如果互质必然有解，如果不互质可能无解。
