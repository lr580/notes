...
Adelson-Velskii 和 Landis ，二叉平衡搜索树 [见wiki](https://oi-wiki.org/ds/avl/)
...
2. 有 $n$ 个非叶子节点的红黑树高度最大为 $h\le 2\log_2(n+1)$
   ...
   从根到叶(不含叶)最少含 $\dfrac h2$ 个黑点，让点数尽可能少，则考虑全是黑点；不可能有路径喊更少的黑点(不然违背 $\dfrac h2$)，则这些路径构成满二叉树，共 $2^{\frac h2}-1$ 个点，即 $n\ge 2^{\frac h2}-1$
