...
- 若 $u$ 等于 $s_i$，匹配 $s_i$ 顺利，将 $u$ 走向中子结点，$i$ 增加
  ...
  直到 $s$ 的每个字符匹配成功且当前节点是终止节点搜索成功，否则搜索失败
...
> $m$ 阶 B 树的[定义](https://oi-wiki.org/ds/b-tree/)：
>
> - 每个节点最多 $m$ 个子节点
>
...
- 分裂后，所有叶子节点不在同一层，所以将分裂出来的子结构往上合并
...
左偏红黑树(LLRB Left Leaning Red Black Tree)是一种容易实现的红黑树变体，定义：
...
> ![image-20230726223322963](img/image-20230726223322963.png)
...
![image-20230726225337367](img/image-20230726225337367.png)
...
![image-20230726231725515](img/image-20230726231725515.png)

以下面例子删除节点 1 为例：

![image-20230726231522067](img/image-20230726231522067.png)
...
2. 有 $n$ 个非叶子节点的红黑树高度最大为 $h\le 2\log_2(n+1)$
...
![image-20230727154645372](img/image-20230727154645372.png)
...
二、要删的是红色叶子节点直接删除；要删的是黑色叶则维护(见下)

三、要删的节点只有左子树或者只有右子树

> 要删的只有一个子树；若 N 是红色直接用子代替它自己；如果 N 是黑色，此时若儿子为红色直接把儿子染黑，否则维护

如果待删节点只有左子树或者只有右子树，因为本来是符合红黑树的，那就表明这个子树必须是黑高平衡的，如果这个要删的点是红色，他还有子树，肯定不满足红黑树，所以要删的点本身是黑色的。然后左子树和右子树只能有一个节点且必然是红色的，所有其他情况都显然不满足红黑树的性质。

因此，对这两种情况，我们就直接把要删的点移除，那么他子树的唯一一个红色的替代上去，改成黑色，仍然满足红黑树的性质。



那么剩下的情况就是要删的节点是黑色的叶子节点。一种可能是本来这个点就是要删的，一种情况是从一推导来的，即交换后等价成了叶子节点要删。

设要删除的黑叶是 x, 其兄是 w：[演示作图工具](https://www.cs.usfca.edu/~galles/visualization/RedBlack.html)

> 为了保证黑高平衡，一定会有兄弟节点



[参考](https://zhuanlan.zhihu.com/p/559541705) or [better](https://zhuanlan.zhihu.com/p/145006031)

经过上述处理后，v 要么是叶子节点(任取空子为u)，要么只有一个子节点(u)

设要删 v，找到替换的叶子节点(或 null)是 u

若 u,v 有且只有一个红色(不可能都是红，不满足红黑树)，直接删 v, u 替换当前位置且设为黑，这种情况上面已经讨论了

![image-20230727223510053](img/image-20230727223510053.png)

否则，u v 都是黑色，将其标记为双黑，并想办法转换为单黑

![image-20230727223610233](img/image-20230727223610233.png)

设兄弟为 s

一、若 s 黑，s 至少一个红

1. LL s 是父 p 左，s 只有左子且红 r 或左右都红：

   操作：r 染黑，s 染为 p 色，右旋 p，p 设为黑，解除双黑

   其中解除双黑的意思是删掉双黑点，p 点(一定是双黑的父)设为黑

   ![image-20230727224213326](img/image-20230727224213326.png)

   ![image-20230727224341493](img/image-20230727224341493.png)

   ![image-20230727224350804](img/image-20230727224350804.png)

   ![image-20230727224401659](img/image-20230727224401659.png)

2. LR s 是 p 左，r 是 s 红右或 s 两个红右

   操作：r 染 p，左旋 s，右旋 p，p 设黑，解除双黑

   ![image-20230727224530091](img/image-20230727224530091.png)

   ![image-20230727224537767](img/image-20230727224537767.png)

   ![image-20230727224547964](img/image-20230727224547964.png)

   ![image-20230727224556561](img/image-20230727224556561.png)

3. RR s 是 p 右，r 是 s 右或两红右

   操作：r 染黑，s 染 p，左旋 p，解双黑

   ![image-20230727224731312](img/image-20230727224731312.png)

   ![image-20230727224744866](img/image-20230727224744866.png)

   ![image-20230727224751133](img/image-20230727224751133.png)

4. RL s 是 p 右，r 是 s 左或 s 两红

   操作：r 染 p，右旋 s，左旋 p，解双黑

   ![image-20230727224946517](img/image-20230727224946517.png)

   ![image-20230727224953173](img/image-20230727224953173.png)

   ![image-20230727225003072](img/image-20230727225003072.png)

   ![image-20230727225010283](img/image-20230727225010283.png)

二、s 黑，两黑子(不管子是不是 NIL)

操作：若 p 是黑，双黑传递给父、s 变红，对父递归处理；直到父红

![image-20230727225345503](img/image-20230727225345503.png)

![image-20230727225405514](img/image-20230727225405514.png)

父红时，兄一定黑，将兄改红，父改黑

![image-20230727225517142](img/image-20230727225517142.png)

![image-20230727225526470](img/image-20230727225526470.png)

三、s 红

则 p 必黑。

- s 是 p 左，对 p 右旋，s,p 变色，然后进入上文其他情况

  ![image-20230727225829423](img/image-20230727225829423.png)

  ![image-20230727225853166](img/image-20230727225853166.png)

  ![image-20230727230050791](img/image-20230727230050791.png)

- s 是 p 右，对 p 左旋，s,p 变色，然后进入上文其他情况

  ![image-20230727230156275](img/image-20230727230156275.png)

  ![image-20230727230204073](img/image-20230727230204073.png)

  ![image-20230727230211101](img/image-20230727230211101.png)
...
![image-20230727230752958](img/image-20230727230752958.png)

判定为 s 红，走 p 左旋和 s,p 换色

![image-20230727230801713](img/image-20230727230801713.png)

优先判定为 s 黑带至少一红，s 是 p 右，两红，故 r 染黑, s 染 p, 左旋 p,解除

![image-20230727231024938](img/image-20230727231024938.png)

![image-20230727231039287](img/image-20230727231039287.png)

![image-20230727231420661](img/image-20230727231420661.png)

下面是废稿：

> 一、w 红
>
> 显然父亲必然是黑，因为要满足黑高，所以 w 必须有两个子节点且一定是黑色的
>
> 操作为：父染红，w 染黑，父左旋
>
> 设删掉下图的 1：
>
> ![image-20230727214515597](img/image-20230727214515597.png)
>
> 父染红，w 染黑，父左旋，得：
>
> ![image-20230727215403753](img/image-20230727215403753.png)
>
> ![image-20230727215822879](img/image-20230727215822879.png)
>
> 二、w 黑，w 左子红右子黑
>
> w 左子染黑，w 染红，w 右旋，从而进入情况三
>
> 
>
> 三、w 黑，w 右子红
...
> 设删掉下图 200：
>
> ![image-20230727222329191](img/image-20230727222329191.png)
>
> 
>
> 四、w 黑，w 左子右子都黑
...
> 维护：
>
> > 设要删的是 N，其兄为 S sibling，父 P，兄的所有子为 nephew C/D
>
> 一、S 是红则 P,C,D 必黑，①N是左子；②N是右子
>
> ①左旋P/②右旋P，S染黑，P染红，之后继续查看下面的情况(从 P 开始)
>
> ![image-20230727161533122](img/image-20230727161533122.png)
>
> 二、N,S 都是黑色，P 是红色
>
> 三、N,S 都是黑色
...
参考 [this](https://blog.csdn.net/weixin_52344401/article/details/125707839)

即一般二叉树的普通的中序遍历思路，故均摊 O1 最坏 logn
...
例题：力扣715 [题解](https://leetcode.cn/problems/range-module/solutions/1612952/by-ac_oier-i4sw/) [我的代码](https://leetcode.cn/problems/range-module/solutions/2525518/c-dong-tai-kai-dian-xian-duan-shu-by-lr5-7f86/)
...
交错路 alternating path 是非匹配点开始，由匹配边与非匹配边交错形成的

增广路 augmenting path 是始于非匹配点且终于非匹配点的交错路

![image-20221126180021096](img/image-20221126180021096.png)

根据 Berge's lemma 当找不到增广路的时候，得到最大匹配。故求最大匹配可以枚举所有未匹配点，找增广路径，直到找不到增广路径。

先考虑增广路算法求二分图最大匹配，复杂度为 $O(nm)$。(网络流可以 $O(\sqrt nm)$)。

因为增广路长度为奇数，路径起始点非左即右，所以我们先考虑从左边的未匹配点找增广路。 注意到因为交错路的关系，增广路上的第奇数条边都是非匹配边，第偶数条边都是匹配边，于是左到右都是非匹配边，右到左都是匹配边。问题转换成，有向图中从给定起点找一条简单路径走到某个未匹配点，此问题等价给定起始点能否走到终点，那么只要从起始点开始 DFS 遍历直到找到某个未匹配点。

对一般图(存在奇环的图)进行最大匹配/最大权匹配。

带花树/开花算法 Blossom Algorithm

显然，奇环直接取反(匹配边和未匹配边对调)会让方案不合理，出现一个点多个匹配边。

![image-20221126175410853](img/image-20221126175410853.png)

一般图增广算法，每次枚举未匹配点，交错染色 `oi`，则 `io` 之间的边是匹配边。

若未访问过 $u$，若其为未匹配点，找增广路经；否则，从其配偶找增广路。否则，即访问过 $u$，遇到标记 $o$ 缩花，否则遇到偶环。缩花后继续找增广路。

![image-20221126180539989](img/image-20221126180539989.png)

实作上找到花以后我们不需要真的缩花，可以用数组纪录每个点在以哪个点为根的那朵花中。

每次找增广路，遍历所有边，遇到花维护花上点，故复杂度为 $O(nm^2)$
...
实际上是 Hierholzer 的思想，vis 的作用是删边。
...
Bron-Kerbosch 算法。复杂度 $O(3^{\frac n3})$。$P$ 是待定，$R$ 是确定，$X$ 是排除
...
算术基本引理：若 $p$ 是素数，且 $p|a_1a_2$，则 $p|a_1,p|a_2$ 至少有一个成立，是素数的本质属性和真正定义。即素数是不可约数。
...
> $$
> \varphi(a)=\prod \varphi(p_i^{k_i})=\prod p_i^{k_i}-p_i^{k_i-1}=\prod p_i^{k_i-1}(p_i-1)=\prod\dfrac{p_i^{k_i}}{p_i}(p_i-1)\\=a\prod\dfrac{p_i-1}{p_i}=a\prod(1-\dfrac{1}{p_i})\\
> \therefore\varphi(a^b)=\prod\dfrac{p_i^{bk_i}}{p_i}(p_i-1)=a^b\prod(1-\dfrac{1}{p_i})=a^{b-1}a\prod(1-\dfrac{1}{p_i})=a^{b-1}\varphi(a)
> $$
...
若 $b\neq 0$，有 $\gcd(a,b)=\gcd(b,a\bmod b)$，因此，若有：
$$
...
联立得：
$$
ax+by=bx'+(a\bmod b)y'=bx'+(a-\lfloor\dfrac ab\rfloor b)y'
$$
这样可以解出一组解。



对 $ax+by=\gcd(a,b)$，若 $b\neq 0$，解出来的可行解必然满足 $|x|\le b,|y|\le a$

对 $ax+by=c$，若 $\gcd(a,b)\nmid c$ 无解。否则，求解 $ax+by=\gcd(a,b)$ 能得到：
$$
a\dfrac{x_0c}{\gcd(a,b)}+b\dfrac{y_0c}{\gcd(a,b)}=c
$$
设一组特解 $x_1=\dfrac{x_0c}{\gcd(a,b)},y_1=\dfrac{y_0c}{\gcd(a,b)}$，则通解为：
$$
a(x_1+db)+b(y_1-da)=c,d\in \Q
$$
设取得最小正整数时为 $x_2,y_2$，作差得：
$$
a(x_1-x_2)+b(y_1-y_2)=0
$$
保证整数，有 $x_1-x_2=kb$ 即 $x_2=x_1\bmod b$。特判 $x_2=0$ 时将其扩大到 $x_2=b$。那么解方程 $ax_2+by_2=c$ 得 $y_2=\dfrac{c-ax_2}{b}$。$x$ 最小时 $y$ 最大。同理解得 $y$ 的最小值，则 $x$ 最大。
...
求 $a^x\equiv b\pmod p$，设 $t=\lceil\sqrt p\rceil,0\le j\le t-1$，设 $x=it-j$，显然可以覆盖所有 $0\le x\le p$。原式为 $a^{it-j}\equiv b\pmod p$ 即 $(a^t)^i\equiv ba^j\pmod p$。预处理出 $a^t$，籍此计算出全部的 $ba^j$。通过哈希表，一次枚举 $a^{ti}$ 时可以直接查询其是否与此前任意一个 $ba^j$ 相等，相等则有解。

若 $a,p$ 不互质，设 $g=\gcd(a,p)$，根据同余的性质，有 $\dfrac{a^x}{g}\equiv \dfrac bg\pmod {\dfrac pg}$，如果 $g\nmid b,b\neq 1$ 都满足，则无解。这是因为子问题的解是 $a',b'$ 的话，原问题的解是 $a=a'g,p=p'g$，代回扩欧，有 $a'^xg^x+yp'g=b$，即 $g(a'^xg^{x-1}+yp')=b$ 即 $g|b$，当且仅当 $b=1$ 时有特解 $a^0=1$，否则都无解，因为 $g\nmid b$ 矛盾。则有：$a^{x-1}\dfrac ad\equiv\dfrac bg\pmod{\dfrac pg}$，不断约分直到互质设 $v=\prod_{i=1}^k\dfrac{a}{g_i}$，则原式为 $a^{x-k}\equiv \dfrac{b}{\prod_{i=1}^kvg_i}\pmod{\dfrac p{\prod_{i=1}^kg_i}}$，将这条东西求解即可。

#### 费马小定理

若 $p$ 为素数且 $\gcd(a,p)=1$，则 $a^{p-1}\equiv 1\pmod p$ 即 $a^p\equiv a\pmod p$ 
...
> 推论：对素数 $p$ 和正整数 $q$ 有 $(p^q!)$

#### 中国剩余定理

构造正确性的证明：设 $m=\prod_{i=1}^pa_i$，令 $m_i=\dfrac{n}{n_i}$，记 $m_i$ 在 $n_i$ 下逆元为 $m_i^{-1}$，则构造 $n=\sum_{i=1}^pb_im_im_i^{-1}$，代回每一个式子，发现对 $\bmod a_j$ 时，$m_i$ 只有 $i=j$ 项不含 $a_j$ 因子，故 $n\bmod a_j=b_im_jm_j^{-1}=b_i$(根据逆元定义，$m_jm_j^{-1}\bmod n_j=1$)，故所有题设满足。

这样构造出的 $n$ 一定是一个解，但不一定是最小解，可以通过模 $n$ 运算转化为最小解。

#### 扩展中国剩余定理

CRT 能使得只要对不同的 $a_i$ 都有不一样的 $x$，即 $a_i$ 与 $x$ 一一对应。且 CRT 易证正确性(最小性由通解和模意义可知)。

对 $p$ 不互质，设：$x\equiv a_1\pmod{m_1},x\equiv a_2\pmod{m_2}$，转化为 $x=m_1p+a_1=m_2q+a_2$，作差得 $m_1p-m_2q=a_2-a_1$。根据裴蜀定理，若 $a_2-a_1$ 不能被 $\gcd(m_1,m_2)$ 整除无解，否则，求出两个方程的解，$x\equiv m_1p+a_1=b\pmod{ m=lcm(m_1,m_2)}$。不断两两方程相邻这么做，直到求出来为止。

具体为，设第一组方程解为 $m=p_1,$ 答案为 $ans=a_1$，然后每次 exgcd 求解：$mx+p_iy$ 的 $x,y$ 和 $g=\gcd(m,p_i)$，如果 $c=a_i-ans$ 不是 $g$ 的倍数就无解。否则，根据 exgcd 可知，同乘 $\dfrac cg$ 得到真实解 $x$。则 $ans=ans+ m\cdot x+ m'$，其中 $m'$ 是新的 $lcm$，即 $m'=\dfrac{m'p_i}{g}$。

根据这个原理，可以求解出其他形式的 exCRT 方程组。

换一种方法来理解，设上一个答案是 $ans$，则因为 $m$ 是之前的 $lcm$，所以加多少倍 $m$ 都不会影响之前的答案的正确性。 然后要保证现在的答案的正确性加上 $x$。考虑 $ans+Mx\equiv a_i\pmod {p_i}$，一眼扩欧，求出 $x$，拼上去。



#### 扩展卢卡斯定理

求 $C_n^m\bmod p$，将 $p$ 质因数分解为 $p_1^{\alpha_1}p_2^{\alpha_2}\cdots$，用中国剩余定理求方程组 $C_n^m\bmod p_1^{\alpha_i}$ 的最小解 $x$。逆元 $m!^{-1}\bmod p_i^{\alpha_i}$ 不一定有解(质数的次方不是质数)。将分子母三个阶乘 $n!$ 同乘以某个 $p_i^v$ 使其有解，得 $\dfrac{n!\div p^x}{m!\div p^y\cdot (n-m)!\div p^z}p^{x-y-z}\bmod p^k$。
...
环 ring 是 $R$ 和两个二元运算 $+,\cdot$ 组成的，且 $(R,+)$ 为交换群，单位元为 $0$，加法逆元为 $-a$；$(R,\cdot)$ 为半群，满足分配律 distributivity，即 $\forall a,b,c\in G,a\cdot(b+c)=a\cdot b+a\cdot c,(a+b)\cdot c=a\cdot c+b\cdot c$。环论。
 ...
 $\cdot $ 满足交换律是交换环commutative ring；存在 $\cdot$ 单位元是幺环ring with identity，幺环存在乘法逆元 $a^{-1}$ 是除环division ring
...
Fast Walsh-Hadamard Transform [参考](https://www.cnblogs.com/wyb-sen/p/15779952.html)

类比 FFT，能在 $O(n\log n)$ 解决由长为 $n$ 的数列 $a,b$ 求新数列 $c_k=\sum_{i+j=k}a_ib_j$

将 `+` 运算换成 `&,|,^`，复杂度不变就是 FWT，但要求 $n=2^k$。

类比 FFT 的处理流程：

![img](img/2221225-20220108101424581-2105988165.png)

希望找到一种变换， 使得 $fwt(a)\times fwt(b)=fwt(c)$。
...
ufwt(a')&=merge(\dfrac{ufwt(a_0')+ufwt(a_1')}2,\dfrac{ufwt(a_0')-ufwt(a_1')}2)
\end{cases}
$$
...
例题参考：`offer` 笔记的力扣 982\.按位与为零的三元组。
...
[参考](https://zhuanlan.zhihu.com/p/402992790)

例题 P1819,offer-力扣321
...
3. 重复子问题。子问题的解存储下来，避免重复求解相同的子问题，从而提升效率。
...
树转换为二叉树： [例子](https://blog.csdn.net/kk1kk2kk3kk4/article/details/144730389)
...
> 特点：转换后根节点有右子树。转换后根节点是原本第一棵树的根节点，它的右倾链的每一个点都是原本的一个树的根节点。[参考](https://blog.csdn.net/kk1kk2kk3kk4/article/details/144730389)
>
> 二叉树还原为森林方法还原为树
...
> 以下图为例：
>
> ![image-20220105113919367](img/image-20220105113919367-1744359266445-4.png)
>
...
![image-20211209101321248](img/image-20211209101321248-1744359266445-3.png)
...
![image-20220105142234952](img/image-20220105142234952-1744359266444-1.png)

![image-20220105142247724](img/image-20220105142247724-16413637687691-1744359266445-2.png)

![image-20220105142332946](img/image-20220105142332946-1744359266445-5.png)

![image-20220105142347791](img/image-20220105142347791-1744359266445-8.png)
...
> 如：
>
> ![image-20211216092548552](img/image-20211216092548552-1744359266445-6.png)
>
> ![image-20211216092559320](img/image-20211216092559320-1744359266446-11.png)
>
> ![image-20211216092608776](img/image-20211216092608776-1744359266445-7.png)

> ![image-20220105142509722](img/image-20220105142509722-1744359266445-10.png)
>
> ![image-20220105142520228](img/image-20220105142520228-1744359266445-9.png)
...
>
> ![image-20220105142615588](img/image-20220105142615588-1744359266446-12.png)



> B+树是非叶子节点值也在叶子节点重复出现的树
>
> ![image-20220105152524891](img/image-20220105152524891-1744359266446-16.png)
...
> 牛客：哈希表的平均查找长度主要取决于装填因子α和处理冲突的方法,与表中元素的具体数目n没有直接关系。这是哈希查找的一个重要特点 (根据上表可得)
>
...
>
> ![image-20220105152508932](img/image-20220105152508932-1744359266446-15.png)
...
> 2024年淘天集团春招研发岗笔试 设有一个顺序共享栈storageArray[70]，其中栈X的栈顶指针top1的初值为-1，栈Y的栈顶指针top2的初值为70，通过不断进行入栈操作，直到storageArray数组已满，此时top1 + top2 = 49。问栈X和栈Y各有多少元素已入栈
>
> 解：答案唯一，栈满故 top2-top1=1，且 top1+top2=49，故 top1=25, top2=45
...
> 2024年阿里云秋招算法岗第一批笔试 [src](https://www.nowcoder.com/exam/test/87508166/submission?examPageSource=Company&pid=60928816&testCallback=https%3A%2F%2Fwww.nowcoder.com%2Fexam%2Fcompany%3FquestionJobId%3D10%26subTabName%3Dwritten_page&testclass=%E8%BD%AF%E4%BB%B6%E5%BC%80%E5%8F%91) 在一个算法王国中，一位年轻的字符串骑士正在解锁KMP算法的谜题。已知字符串S='uuluuululululuulppuu'，模式串t='ululuu'，当第一次出现“失配”（s[i]≠t[j]）时，i=j=1。下次开始匹配时，i和j的值分别是（）
>
> 解：next = [-1, 0, 0, 1, 2, 3]，i 是主串，j 是模式串
>
> 初始匹配，i=j=0，成功匹配，然后 i=j=1，然后发现失配，故j=next[j]=0，故i=1,j=0；
>
> 扩展：下次匹配成功，则下次为 i=2,j=1
...
![image-20220104201739000](img/image-20220104201739000-1744359266446-13.png)
...
![image-20220105142454478](img/image-20220105142454478-1744359266446-14.png)



![image-20220105152842561](img/image-20220105152842561-1744359266446-19.png)

![image-20220105152905769](img/image-20220105152905769-1744359266446-18.png)

![image-20220105152918047](img/image-20220105152918047-1744359266446-17.png)
...
> 2024年阿里云秋招算法岗第一批笔试 已知一个有序表X(2, 5, 9, 15, 22, 30, 39, 49, 60)，若采用二分查找算法查找目标值，对每个关键字的查找概率相同，则下列说法中正确的是
>
> 1. 查找一个X中不存在的关键字，查找次数最多是5
> 2. 查找一个X中存在的关键字，查找次数最多不超过4
> 3. 查找成功的平均查找长度为30/9
> 4. 查找失败的平均查找长度为17/5
>
> 解：BD。失败也是4次(不超过树深)。成功 ASL 为 (1+2+2+3+3+3+3+4+4)/9=25/9 (即完全二叉树高序列)，失败 ASL：(25+9)/10=17/5
...
![image-20220105175833039](img/image-20220105175833039-1744359266452-20.png)

![image-20220105175839924](img/image-20220105175839924-1744359266452-21.png)

![image-20220105175852486](img/image-20220105175852486-1744359266452-22.png)

![image-20220105175906646](img/image-20220105175906646-1744359266452-25.png)

![image-20220105175913588](img/image-20220105175913588-1744359266452-23.png)

![image-20220105175924702](img/image-20220105175924702-1744359266452-24.png)

![image-20220105175937830](img/image-20220105175937830-1744359266452-26.png)

![image-20220105175951918](img/image-20220105175951918-1744359266453-28.png)

![image-20220105180003235](img/image-20220105180003235-1744359266453-27.png)
...
> 牛客 一份新闻网站的新闻热度记录在一棵小根堆二叉树中，其中序遍历为{33, 16, 20, 8, 40}，具体二叉树的形态满足小根堆的性质，若将该二叉树看作一个小根堆，则将小根堆调整为大根堆需交换（）次。解：2
>
> 树为：
>
> ```mermaid
...
> ```
>
> 从最后一个非叶子节点开始，一个个点 heapify，其中 16 swap 33, 40 swap 8
