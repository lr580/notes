...
- [chatgpt](https://openai.com/index/introducing-chatgpt-go/) go 8 , plus 20 , pro 200 USD/M [tokens](https://developers.openai.com/api/docs/pricing)
- [claude](https://claude.com/pricing) pro 17, max 5x 100, max 20x 200 USD/M
- [gemini](https://gemini.google/us/subscriptions/?hl=en) plus 8, pro 20, ultra 250 USD/M
- [cursor](https://cursor.com/en-US/pricing)  pro 20, pro+ 60, ultra 200 USD/M

> USD / 1M tokens 计费：
>
> | 官方模型名                                                   |                          输入 |                   缓存输入/缓存相关 |          输出 |
> | ------------------------------------------------------------ | ----------------------------: | ----------------------------------: | ------------: |
> | [gpt-5.3-codex](https://developers.openai.com/api/docs/models/gpt-5.3-codex) |                          1.75 |                               0.175 |         14.00 |
> | [gpt-5.4](https://developers.openai.com/api/docs/models/gpt-5.4) |                          2.50 |                                0.25 |         15.00 |
> | [claude-opus-4-6](https://platform.claude.com/docs/en/about-claude/pricing) |                          5.00 | 5m写入 6.25 / 1h写入 10 / 命中 0.50 |         25.00 |
> | [claude-sonnet-4-6](https://platform.claude.com/docs/en/about-claude/pricing) |                          3.00 |  5m写入 3.75 / 1h写入 6 / 命中 0.30 |         15.00 |
> | [gemini-3.1-pro-preview](https://ai.google.dev/gemini-api/docs/pricing?hl=zh-cn) | 2.00（<=200k）/ 4.00（>200k） |       0.20 / 0.40（另有缓存存储费） | 12.00 / 18.00 |
...
快捷键（高频）
  ...
  - Ctrl+L：只清屏，不重开对话（和 /clear 不同）。

  - Ctrl+C：退出会话（等价 /exit / /quit）。

  - Ctrl+G：用外部编辑器写长 prompt。(需要配置)
  ...
  - Esc 两次（输入框空时）：回到上一条用户消息继续改。（← → 切，enter 删掉(背景色改变)从当前往后的全部信息，重新编辑当前信息）
...
Slash 命令
  ...
  - /model：切模型/推理强度。
  - /permissions：快速切 Auto / Read-only / Full Access。
  - /status：看当前模型、审批策略、token 使用。
  - /compact：长对话后压缩上下文，省 token。
  - /review + /diff：做代码审查并看改动。
  - /new / /resume / /fork：新开、续接、分支化对话。
...
- codex review --uncommitted
  让 Codex 审查你当前仓库里还没提交的改动（staged/unstaged/untracked）。
