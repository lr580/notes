...
必须接收 append 的返回值，否则它会表示返回值未被使用
...
Go 的垃圾回收器依赖于类型安全，使用 `unsafe.Pointer` 可能导致垃圾回收器无法正确追踪对象，从而引发内存泄漏。而且如果内存越界和类型不匹配会导致错误。
...
- 包的初始化顺序：如果一个包被多个包依赖，Go 会首先执行所有导入的包的 `init()` 函数，依赖的包 `init()` 函数会先执行。
- 同一个包多文件多 init，按文件名顺序。
- 同一个文件内多个 init，按出现顺序。多个同名函数 init 是特例。

> 场景例子：sql init，日志记录注册
...
> 常见错误有：越界、空指针解引用、非法类型断言、数字错误(1/0)、调用panic、内存越界/非法操作、运行时错误、不安全的库/代码
>
> Go 更推荐使用显式错误处理，而不是通过 panic 和 recover 来处理常规的错误情况

用 errors 包，可以主动新建错误。但它只是创建错误对象，不会像 panic 一样中断程序执行。
...
> 调用 `panic` 会导致程序的控制流中断，并开始逐层向上传播，直到程序崩溃或被 `recover` 捕获。常使用于程序启动初始化错误处理
...
通过recover 将程序恢复回来， 继续往后执行。 recover 只能在 defer 调用的函数里。
...
构建约束（Build Constraints）。也叫构建标签（Build Tags）。允许你控制哪些 `.go` 文件应该被包含在编译过程中。这使得你可以根据不同的目标操作系统、架构、环境或自定义条件来编译不同版本的代码。

文件级约束：在文件开头使用 `//go:build` + 空行，加一个布尔值表达式条件，如：

忽略其他同文件下的 `.go` 文件，只考虑该文件自己。
...
如果局部变量的生命周期超出函数作用域（如通过指针返回给外部使用），编译器会将变量分配在堆上，确保变量在作用域外仍然有效，这种机制称为“逃逸分析”，触发场景：

- 果局部变量的地址被返回
- 局部变量被闭包捕获
- 局部变量被传递到另一个 Goroutine，则无法保证其安全存储在栈
...
> 		return x
> 	} 
> }// 输出有一行： ./escape.go:7:9: func literal escapes to heap
> 
> func main() {}
> ```
...
得到新数组，也会修改原数组(但原数组长度不变，后面的项不确定是原数组的项，可能是0)
...
`cmp.Compare`比较两个有序类型的值 `x`和 `y`，并返回：
