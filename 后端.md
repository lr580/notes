...
所用函数通常有：`count, sum, avg` 。聚合函数不能进行运算，如 `>80` 。
...
做数据表连接的话需要 `from ... where ...` ，其from的表可以是子表等
...
current\_time 或 curtime 是当时的时分秒
...
> ```
>
> 运算符：逻辑用 AND OR NOT ，其他类比一般操作运算符。字符串连接用 + 。支持位运算 `&|~^` 。
>
> 
>
> #### 流程控制
