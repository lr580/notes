...
只有 public 类型的方法才是可以通过URL访问的。要调用哪个方法，就直接 HTML 里打这个方法名，如： `index.php/index/index/hello`  (假设在上述代码定义了这个函数)



##### 视图

在 `application/模块名` 创建目录 `view` ，尝试添加一个 `html` ，随便写点内容，如果有变量，用 `{$变量名}` 表示占位。
