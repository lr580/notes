...
- 3464\.正方形上的点之间的最大距离
  ...
  二分套二分+枚举 / <u>+双指针</u> / <u>+倍增+预处理</u> / <u>+DFS+栈</u> / <u>+DP</u>
...
class Solution {
    public String longestPalindrome(String s) {
        int n = s.length();
        boolean [][]dp = new boolean[n][n]; // whether s[l:r] is palidrone   
        int maxLen = 1; // max r-l+1 if dp = true
        int ansL = 0;
        for(int i=0;i<n;i++) { // init len=1
            dp[i][i] = true; 
        }
        for(int i=0;i<n-1;i++) { // init len=2
        ...
        for(int len=3;len<=n;len++) { // interval DP
            for(int l=0, r=len-1; r<n; l++, r++) {
        ...
        for(int i=ansL;i<ansL+maxLen;i++) {
...
- 计算 cnt 的自卷积 cnt2，其 `cnt2[i]` 表示 a+b=i 的方案数，即：
  $$
  cnt_2[k]=\sum_icnt_i\cdot cnt_{k-i}
  $$
  也就是对多项式 cnt，求 cnt2 = cnt*cnt。

- 如果是偶数，i/2+i/2=i，然而不能自己加自己。

  设 $I=cnt[i/2]$，$I$ 里不使用自己拼出 i 的方案显然是 $I(I-1)$。但是实际计算的数量是 $I^2$，所以多算了 $I^2-I(I-1)=I$，所以要减去 $I$。

- 由于 a+b=b+a，去重后结果一定是 cnt2 的一半。

答案为全体方案减去不合法方案。全体方案是 $C_m^3$，其中 $m$ 是原数组长度 $m=|nums|$ (需要把数值0剔除，0不能构成边长)。

不合法方案就是 $a+b\le c$。设经过上述修正后的 $cnt2$ 的前缀和是 $sum$，显然 $\le c$ 的二元组的个数是 $\sum_{i=0}^c cnt_2[i]=sum[c]$。而 $c$ 有 $cnt[c]$ 个，故对 $c$ 的不合法数有 $sum[c]\cdot cnt[c]$。故值域大小是 $U=\max nums$，总不合法数为 $\sum_{i=1}^Usum[i]\cdot cnt[i]$。
...
[题目](https://leetcode.cn/problems/minimum-distance-to-type-a-word-using-two-fingers)

我的代码

```java
class Solution {
    static private int[] getPos(int v) {
        return new int[]{v%6,v/6};
    }
    static private int getDis(int c1, int c2) {
        int[] p1 = getPos(c1), p2 = getPos(c2);
    ...
    public int minimumDistance(String word) {
        ...
        for(int j=0;j<26;j++) {
            for(int k=0;k<26;k++) {
                ...
                dp[0][j][k] = Math.min(getDis(v, j), getDis(v, k));
            }
        }
        for(int i=1;i<n;i++) {
            ...
            for(int j=0;j<26;j++) {
                for(int k=0;k<26;k++) {
                    dp[i][v][k] = Math.min(dp[i][v][k], dp[i-1][j][k] + getDis(j, v));
                    dp[i][j][v] = Math.min(dp[i][j][v], dp[i-1][j][k] + getDis(k, v));
                }
            }
        }
        int ans = Integer.MAX_VALUE;
...
> 因为时间关系，大部分题目只看了没做，所以不记录题目链接在这里了。
...
public:
    int findMin(vector<int>& a) {
        ...
        // vector<int> a = {4,4,4,4,4,4,4,4,4,2,4,4,4,4,4,4};//{4,4,4,4,4,4,4,4,4,4,4,4,4,4,2,4};
        // vector<int> a = {1,2,3,4,5};
        // vector<int> a = {3,3,4,5,5,1,1,1,2,2};
        // vector<int> a = {5,1,2,3,4};
        int n = a.size();
        // {3,4,5,1,2}
        // 3>4 3>5 3>1 3>2
        // false false true true
        // target: first index of true
        int l = 0, r = n-1, ans = -1;
        // SPJ
        // case 1 {1,2,3,4,5} all false 
        // case 2 {5,1,2,3,4} OKay
        // case 3 {4,4,2,4,4}
        // > / >= f f t f f
        // < f f f f f    <= t t f t t
...
public:
    int kthSmallest(vector<vector<int>>& matrix, int k) {
        ...
        while (k--) {
            ...
            answer = -smallest.val;
            // cout << answer << "\n"; // debug
            int i = smallest.row, j = smallest.col;
            if (j + 1 < n) {
                node nd = node{-matrix[i][j + 1], i, j + 1};
                pq.push(nd);
            }
        }
        return answer; 
