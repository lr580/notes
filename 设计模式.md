...
public class IgoChessmanWhite extends IgoChessman {
    @Override
    public String getColor() {
        return "白";
    }
...
public class IgoChessmanFactory {
    ...
    private IgoChessmanFactory() {
        // 初始化单例存于ht中
        ht = new HashMap<>();
        ht.put("w", new IgoChessmanWhite());
        ht.put("b", new IgoChessmanBlack());
    }
...
2. 虚拟代理。
   ...
   设计一个与真实对象有相同接口的虚拟对象。如果真实对象已经创建则代理对象转发给真实对象，否则先创建。
...
特点：职责相同，程度不同。[力扣](https://leetcode.cn/leetbook/read/job-hunting-all-in-one-school/0sk3n5/)
...
动机：一个对象发成改变时自动通知其他对象，其他对象将相应做出反应。一个观察目标可以对应多个观察者，而且这些观察者之间没有相互联系，可以根据需要增加和删除观察者，使得系统更易于扩展。
...
一个对象取决于一个或多个动态变化的属性(状态)，这种对象叫有状态的stateful对象。状态从定义好的一系列值取出，可以用UML状态图描述。
...
> 牛客：现在要写一个爬虫服务，比如网站A你要进行爬虫操作A，网站B你要进行爬虫操作B
...
动机：为不同类型的元素提供多种访问操作方式，且可以在不修改原有系统的情况下增加新的操作方式。
